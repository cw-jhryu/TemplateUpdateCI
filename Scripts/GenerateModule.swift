...
func registerModuleDependency() {
    checkModuleExists(filePath: "Projects/\(layer.rawValue)/\(moduleName)/Project.swift")
...
func checkModuleExists(filePath: String) {
    let fileURL = URL(fileURLWithPath: filePath)
    let readHandle = try? FileHandle(forReadingFrom: fileURL)
    guard readHandle == nil else {
        fatalError("❌ Already Exists \(filePath)")
    }
}
...
print("Enter layer name\n(Feature | Domain | Data | Core | Shared | UserInterface)", terminator: " : ")
